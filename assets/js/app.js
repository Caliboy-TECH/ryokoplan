window.RyokoApp = (() => {
  ...
  const editorialData = {
    magazine: {
      ko: {
        ...
        cityMeta: {
          tokyo:'Japan edit · Fast city', osaka:'Japan edit · Food / easy fun', kyoto:'Japan edit · Slow trip', fukuoka:'Japan edit · Compact food trip', sapporo:'Japan edit · Snow-lit city', sendai:'Japan edit · Green city rest', okinawa:'Japan edit · Sea-breeze island',
          seoul:'Korea edit · Fast city', busan:'Korea edit · Coast / food', jeju:'Korea edit · Coast / nature', gyeongju:'Korea edit · Slow heritage trip', taipei:'Greater China edit · Night food layers', hongkong:'Greater China edit · Vertical harbor nights', macau:'Greater China edit · Compact heritage nights'
        },
        cityCopy: {
  ...
  function renderCityPage(){
    ...
    root.innerHTML = `
      ...
      <section class="section city-quicknav-wrap"><div class="city-quicknav"><a class="jump-chip" href="#city-overview">${uiText('overview')}</a><a class="jump-chip" href="#city-districts">${uiText('districts')}</a><a class="jump-chip" href="#city-sample">${uiText('sample')}</a><a class="jump-chip" href="#city-tips">${lang === 'ko' ? 'Tips' : 'Tips'}</a></div></section>
      <section class="section city-overview-composition" id="city-overview"><div class="city-overview-lead"><div class="editorial-kicker">${uiText('overview')}</div><h2 class="section-title">${lang === 'ko' ? entry.planner + ' at a glance' : entry.planner + ' at a glance'}</h2><p class="section-desc">${data.whyDesc}</p></div><div class="city-meta-strip"><article class="meta-card feature"><span class="meta-label">${lang === 'ko' ? 'Best for' : 'Best for'}</span><span class="meta-value">${data.bestFor}</span></article><article class="meta-card"><span class="meta-label">${lang === 'ko' ? 'Suggested pace' : 'Suggested pace'}</span><span class="meta-value">${data.pace}</span></article><article class="meta-card"><span class="meta-label">${uiText('bestSeason')}</span><span class="meta-value">${data.season}</span></article></div></section>
      ...
      <section class="section city-reading-grid city-reading-grid-rich" id="city-tips"><article class="info-card editorial-panel"><div class="section-head compact"><div><div class="editorial-kicker">${uiText('localNotes')}</div><h2 class="section-title">${uiText('localTips')}</h2><p class="section-desc">${lang === 'ko' ? '실제로 여행 체감을 바꾸는 작은 포인트들입니다.' : 'Small adjustments that noticeably improve the trip.'}</p></div></div><ul class="editorial-bullets">${data.tips.map(item => `<li>${item}</li>`).join('')}</ul></article><article class="info-card editorial-panel editorial-panel-soft"><div class="section-head compact"><div><div class="editorial-kicker">${uiText('beforeYouGo')}</div><h2 class="section-title">${uiText('keepInMind')}</h2><p class="section-desc">${lang === 'ko' ? '출발 전에 챙기면 바로 체감되는 것들입니다.' : 'Small prep choices that pay off immediately.'}</p></div></div><ul class="editorial-bullets">${data.keep.map(item => `<li>${item}</li>`).join('')}</ul></article></section>
  ...
  function renderExamplePage(){
    ...
    root.innerHTML = `
      ...
      <section class="section city-overview-composition"><div class="city-overview-lead"><div class="editorial-kicker">${uiText('atAGlance')}</div><h2 class="section-title">${lang === 'ko' ? '샘플 구조를 먼저 읽어보세요' : 'Read the structure first'}</h2><p class="section-desc">${lang === 'ko' ? '이 예시는 장소 리스트보다 하루 리듬을 가져가는 데 더 큰 가치가 있습니다.' : 'This example is more useful as a pacing reference than as a strict checklist.'}</p></div><div class="city-meta-strip"><article class="meta-card feature"><span class="meta-label">${lang === 'ko' ? 'Best for' : 'Best for'}</span><span class="meta-value">${cityData.bestFor}</span></article><article class="meta-card"><span class="meta-label">${lang === 'ko' ? 'Suggested pace' : 'Suggested pace'}</span><span class="meta-value">${cityData.pace}</span></article><article class="meta-card"><span class="meta-label">${lang === 'ko' ? 'Budget feel' : 'Budget feel'}</span><span class="meta-value">${cityData.budgetFeel}</span></article></div></section>
...
function getSeasonalEditorialCollections(){
  ...
  const magazineLangPatches = {
    ja: {
      heroChips:['East Asia city edit','日本・韓国・中華圏','planner-ready routes'],
      countryJapan:'日本', countryKorea:'韓国', countryGreaterChina:'中華圏',
      guideBtn:'ガイド', planBtn:'プラン'
    },
    zhHant: {
      heroChips:['East Asia city edit','日本・韓國・大中華','planner-ready routes'],
  ...
  function initEditorialChrome(){
    ...
    const labels = {
      ...
      ja: {
        planner: { kicker: 'Planner ノート', title: '街のムードから始めて、保存したくなる旅へつなげます。', chips: ['East Asia city edit', 'Magazine主導の設計', '保存 / 共有 / PDF'] },
        magazine: { kicker: 'Magazine ノート', title: '都市の読みが良いほど、旅の流れも自然になります。', chips: ['都市ガイド', 'サンプルルート', 'ローカルのリズム'] },
        trips: { kicker: 'My Trips ノート', title: '保存した旅程と共有された旅程を、流れを切らさずにもう一度開けます。', chips: ['保存した旅', '最近のプラン', '共有リンク'] }
      },
      zhHant: {
        planner: { kicker: 'Planner 筆記', title: '先抓住城市氛圍，再接到值得保存的旅程。', chips: ['East Asia city edit', 'Magazine 主導規劃', '儲存 / 分享 / PDF'] },
        magazine: { kicker: 'Magazine 筆記', title: '城市讀得越清楚，旅程也會排得更自然。', chips: ['城市指南', '範例路線', '在地節奏'] },
        trips: { kicker: 'My Trips 筆記', title: '把已儲存和共享的旅程放在同一條流裡重新打開。', chips: ['已儲存旅程', '最近計畫', '共享連結'] }
      }
    };
