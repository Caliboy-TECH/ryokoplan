window.RyokoApp = (() => {
  ...
  function getCityGrowthHook(slug='', entry={}){
    ...
    const base = cityGrowthHookMap[key] || { hook:`${entry.planner || 'This city'} reads better before it becomes a route.`, search:'City-first itinerary with neighborhoods, pace, weather pivots, and sample routes.', share:'A route built from city mood instead of a random checklist.', save:'Best when you want the trip to feel edited and easy to return to.' };
    ...
    const packs = {
      ...
      ja: { eyebrow:'City hook', title:`${city} は、ルートにする前に街のテンポを読む都市です`, desc:'検索やSNSから入っても、この都市をどう読み、どこから route へ進むかがすぐ分かるように整えました。', search:'検索で伝える一文', share:'共有で伝える一文', save:'保存する理由', hook:`${city} は、場所の数より街のテンポが先に合うと旅が整います。`, searchLine:`${city} itinerary · neighborhoods · weather pivots · sample route`, shareLine:`${city} をチェックリストではなく、ひとつの旅の流れとして読むガイド`, saveLine:'あとで開いても、そのまま route づくりへ戻れます。' },
      zhHant: { eyebrow:'City hook', title:`${city} 要先讀城市節奏，再變成路線`, desc:'無論從搜尋或社群進來，都能先看懂這座城市適合怎麼讀、為什麼值得保存、以及下一步怎麼開始。', search:'搜尋句', share:'分享句', save:'保存理由', hook:`${city} 不是先塞景點，而是先抓住城市節奏才會清楚。`, searchLine:`${city} itinerary · neighborhoods · weather pivots · sample route`, shareLine:`把 ${city} 當成一條城市節奏來讀，而不是一份景點清單`, saveLine:'之後再打開，也能直接接回 sample 與 planner。' }
    };
...
editorialData.example['macau-2n3d-night-lanes'] = { titleKo:'Macau 2박 3일 night lanes', titleEn:'Macau 2N3D Night Lanes', city:'Macau', image:'assets/images/examples/macau-heritage-night.jpg', guide:'macau.html', koLead:'마카오의 old lanes와 짧은 night glow를 조금 더 골목 중심으로 읽는 샘플입니다.', enLead:'A Macau sample that leans more into old lanes and one cleaner night glow instead of broad spectacle.', ko:{ routeShape:'old-lane entry → Taipa bridge → 짧은 night glow', energyControl:'하루를 길게 끌지 않고, 걷는 축 하나와 밤 장면 하나만 남깁니다.', swapNote:'비가 오면 heritage walk를 줄이고 indoor tea / dessert pocket을 더 길게 쓰세요.', bestWith:'커플, 부모님, 짧은 heritage trip', whyBullets:['마카오는 골목을 짧게 읽을수록 더 또렷합니다.','Taipa가 old core와 night side 사이를 부드럽게 이어줍니다.','한 장면의 밤으로도 충분히 contrast가 남습니다.'] }, en:{ routeShape:'Old-lane entry → Taipa bridge → short night glow', energyControl:'Keep the day short and leave only one walking axis plus one night scene in focus.', swapNote:'When rain arrives, reduce the heritage walk and extend one indoor tea or dessert pocket instead.', bestWith:'Couples, parents, short heritage trips', whyBullets:['Macau often gets clearer when its lanes are read briefly and well.','Taipa softens the jump between the old core and the brighter night side.','One night scene is enough to hold the contrast.'] } };
  ...
  function getExpansionCityDeepDiveCopy(slug){
    const labelMap = {
      ...
      en:{eyebrow:'Expansion city deepening',title:'Read the expansion cities by travel rhythm',desc:'The expansion cities now carry a complete first-trip, return-trip, rainy-day, slower-day, and night-rhythm layer.',first:'First trip',ret:'Return trip',rainy:'Rainy day',slow:'Slower day',night:'Night rhythm',ctaGuide:'Read sample route',ctaPlan:'Start with this city'},
  ...
  function renderCityRouteHandoff(slug, entry){
    ...
    const labels = {
      ...
      desc: lang === 'ko' ? '이 페이지는 끝나는 가이드가 아니라, 샘플 일정과 플래너로 넘어가는 출발점입니다.' : lang === 'ja' ? 'このページは読み終えるだけのガイドではなく、sample と planner へ進む入口です。' : lang === 'zhHant' ? '這一頁不是讀完就結束的指南，而是接到 sample 與 planner 的入口。' : 'This guide is not a dead end. It hands the city mood into a sample route, then into your own plan.',
      ...
      sampleDesc: lang === 'ko' ? '장소 리스트보다 하루의 리듬을 먼저 봅니다.' : lang === 'ja' ? '場所リストより、一日のリズムを先に見ます。' : lang === 'zhHant' ? '先看一天的節奏，而不是單純看清單。' : 'Read the day rhythm before copying the list.',
