window.RyokoApp = (() => {
  ...
  const cityLoopMap = {
    ...
    taipei: { name:'Taipei', country:'Taiwan', guide:'city/taipei.html', example:'example/taipei-3n4d-night-food.html', image:'assets/images/cities/taipei.jpg', vibe:'food night' },
    hongkong: { name:'Hong Kong', country:'Hong Kong', guide:'city/hongkong.html', example:'example/hongkong-3n4d-harbor-rhythm.html', image:'assets/images/cities/hongkong.jpg', vibe:'harbor city' },
    macau: { name:'Macau', country:'Macau', guide:'city/macau.html', example:'example/macau-2n3d-heritage-night.html', image:'assets/images/cities/macau.jpg', vibe:'heritage night' }
  };
  const loopPairs = {
  ...
  cityVoiceMap.sapporo = {
    ko: { strap:'계절감이 장면을 만드는 북쪽 도시.', reward:'한 구간씩 넓게 읽을수록 좋음', watch:'하루에 너무 많은 이동 축을 넣지 않기', mood:'crisp / calm / winter-soft' },
    en: { strap:'A northern city where season becomes the scene.', reward:'It pays off when each zone gets more room', watch:'Do not stack too many movement axes in one day', mood:'crisp / calm / winter-soft' }
  };
  cityVoiceMap.sendai = {
    ko: { strap:'조용한 도시 결을 읽는 데 강한 베이스.', reward:'부담 적은 도시 템포가 오래 남음', watch:'도심과 근교를 같은 날 깊게 넣지 않기', mood:'quiet / balanced / local' },
    en: { strap:'A strong base for reading quieter city texture.', reward:'The lighter city tempo stays with you longer', watch:'Do not go deep on downtown and side trips on the same day', mood:'quiet / balanced / local' }
  };
  cityVoiceMap.okinawa = {
    ko: { strap:'섬에서는 장소보다 바람과 여유가 먼저입니다.', reward:'바다와 쉬는 시간의 조합이 강함', watch:'체크리스트처럼 해변을 많이 넣지 않기', mood:'airy / coast / easy' },
    en: { strap:'On the island, air and ease matter before stop count.', reward:'Sea views work best with real rest windows', watch:'Avoid turning beaches into a checklist', mood:'airy / coast / easy' }
  };
  ...
  citySectionMap.taipei = {
    ko: { moduleEyebrow:'Taipei edit', moduleTitle:'타이베이는 음식과 밤 리듬을 같이 읽어야 좋습니다', moduleDesc:'큰 랜드마크보다 먹는 간격, 골목, 늦은 시간의 템포를 어떻게 묶느냐가 더 중요합니다.', modules:[{type:'timing', title:'Best windows', items:[['오후 늦게','카페와 골목, 첫 야시장 전 리셋'],['저녁','야시장 또는 식사 축 한 곳을 깊게'],['늦은 밤','강한 이동보다 늦은 카페나 산책으로 마감']]},{type:'pair', title:'Good pairings', items:[['동네 산책 → 야시장','먹는 흐름이 자연스럽게 이어짐'],['카페 → 전망/야경','밤 템포가 더 선명해짐'],['한 구역 집중 + 다음 날 다른 결','과밀하지 않고 만족도 높음']]}] },
    en: { moduleEyebrow:'Taipei edit', moduleTitle:'Taipei works best when food rhythm and night pace are read together', moduleDesc:'More than headline landmarks, the city improves when meals, alleys, and after-dark pacing are edited well.', modules:[{type:'timing', title:'Best windows', items:[['Late afternoon','Cafés, alleys, and one reset before the night market'],['Evening','Go deeper on one market or one dinner axis'],['Later at night','Close with a softer café or walk, not another long transfer']]},{type:'pair', title:'Good pairings', items:[['Neighborhood walk → night market','The food rhythm opens naturally'],['Café → night view','The city feels sharper after dark'],['One focused zone a day','Higher payoff than stacking too much']]}] }
  };
  ...
  function buildDiscoveryItems(){
    ...
    const items = [
      ...
      { kind:'city', slug:'taipei', title:{ko:'Taipei night food base',en:'Taipei night food base'}, desc:{ko:'먹는 리듬과 밤의 무드를 중심으로 읽는 타이베이 베이스.',en:'A Taipei base built around food rhythm and softer night energy.'}, tags:['taipei','taiwan','food','night','friends','weekend'], guide:`${guideBase}city/taipei.html`, example:`${guideBase}example/taipei-3n4d-night-food.html`, preset:{destination:'Taipei',duration:'3n4d',companion:'friends',style:'food-led neighborhoods + late cafés',notes:'Keep the night strong and the daytime route lighter.'}},
      { kind:'city', slug:'hongkong', title:{ko:'Hong Kong harbor break',en:'Hong Kong harbor break'}, desc:{ko:'항구 장면과 밤 리듬을 짧고 강하게 남기는 홍콩 베이스.',en:'A Hong Kong base shaped around harbor scenes and sharper nights.'}, tags:['hongkong','harbor','night','city','couple','weekend'], guide:`${guideBase}city/hongkong.html`, example:`${guideBase}example/hongkong-3n4d-harbor-rhythm.html`, preset:{destination:'Hong Kong',duration:'3n4d',companion:'couple',style:'harbor views + food + night city',notes:'Do fewer districts well and protect one afternoon reset.'}},
      { kind:'edit', slug:'macau-short', title:{ko:'Macau compact heritage',en:'Macau compact heritage'}, desc:{ko:'짧은 체류에 맞춰 heritage와 night를 compact하게 읽는 마카오 베이스.',en:'A compact Macau base for shorter stays built around heritage and one clear night close.'}, tags:['macau','heritage','night','parents','compact'], guide:`${guideBase}city/macau.html`, example:`${guideBase}example/macau-2n3d-heritage-night.html`, preset:{destination:'Macau',duration:'2n3d',companion:'family',style:'heritage walks + local food + soft nights',notes:'Keep one walkable core and do not over-expand the route.'}}
    ];
    return items;
  }
...
function getSeasonalEditorialCollections(){
  ...
  function renderHomeDiscovery(){
    ...
    const copyMap = {
      ...
      ja: {
        eyebrow:'Quick find', title:'ムードや状況からすぐ絞り込む', desc:'都市名が先に浮かばなくても大丈夫です。週末、雨の日、親との旅のような状況から始められます。',
        placeholder:'都市・mood・状況で検索', all:'All', weekend:'Weekend', rainy:'Rainy day', parents:'With parents', food:'Food-led', coast:'Coast', empty:'まだ決まらなければ、先に city magazine を読んでください。',
        guide:'City guide', sample:'Sample route', plan:'Use as base', matches:'件', helper:'Suggested searches'
      },
      zhHant: {
        eyebrow:'Quick find', title:'用 mood 或情境快速縮小', desc:'就算你還沒想到城市名，也可以先從週末、雨天、和父母同行這種情境開始。',
        placeholder:'用城市、mood、情境搜尋', all:'All', weekend:'Weekend', rainy:'Rainy day', parents:'With parents', food:'Food-led', coast:'Coast', empty:'如果還沒有合適的，就先去讀 city magazine。',
        guide:'City guide', sample:'Sample route', plan:'Use as base', matches:'個結果', helper:'Suggested searches'
      }
    };
    const copy = copyMap[lang] || copyMap.en;
    root.innerHTML = `
    ...
    const maps = {
      planner: {
        ja: {
          ...
          '.brand-manifesto-line:nth-of-type(1) strong':'Read the city',
          ...
          '.brand-manifesto-note p':'Read the city. Then build the trip. この一文を軸に、ホーム・マガジン・都市ガイド・結果画面を同じトーンでつなぎます。',
      ...
      magazine: {
        ...
        zhHant: {
          '.magazine-shell .utility-banner .eyebrow':'Planner note',
  ...
  editorialData.magazine.ja = {
    ...
    cityMeta:{tokyo:'Japan · Fast city', osaka:'Japan · Food / easy fun', kyoto:'Japan · Slow trip', fukuoka:'Japan · Compact food trip', seoul:'Korea · Fast city', busan:'Korea · Coast / food', jeju:'Korea · Coast / nature', gyeongju:'Korea · Slow heritage trip', taipei:'Taiwan · Night food city', hongkong:'Harbor city · Sharp nights', macau:'Macau · Heritage night'},
