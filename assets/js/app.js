window.RyokoApp = (() => {
        ...
        <div class="cover-desk-grid cover-desk-grid-magazine">
          <article class="cover-story-card cover-story-card-large">
            ...
            <div class="cover-story-body">
              ...
              <div class="cta-row"><a class="primary-btn" href="../city/tokyo.html">${lang === 'ko' ? '도시 가이드 열기' : lang === 'ja' ? '都市ガイドを開く' : lang === 'zhHant' ? '打開城市指南' : 'Open city guides'}</a><a class="secondary-btn" href="../example/tokyo-3n4d-first-trip.html">${lang === 'ko' ? '샘플 루트 보기' : lang === 'ja' ? 'サンプルルートを見る' : lang === 'zhHant' ? '看範例路線' : 'See a sample route'}</a></div>
            </div>
          </article>
      ...
      <section class="section editorial-dispatch-section magazine-dispatch-section">
        ...
        <div class="dispatch-edit-grid">
          ...
          <article class="dispatch-edit-card">
            ...
            <p>${lang === 'ko' ? '실내 밀도와 동네 이동을 가볍게 유지하는 서울 우천일 베이스입니다.' : lang === 'ja' ? '室内の密度を保ちながら、街区の移動を軽くつなぐソウルの雨の日ベースです。' : lang === 'zhHant' ? '這是把室內密度和街區移動都維持得更輕一些的首爾雨天基底。' : 'A Seoul fallback that keeps the day indoor-friendly while staying neighborhood-led.'}</p>
  ...
  function renderCityPage(){
    ...
    root.innerHTML = `
      ...
      <section class="section city-section-band"><article class="cover-story-card story-band"><div class="editorial-kicker">${lang === 'ko' ? '편집 포인트' : lang === 'ja' ? '編集の視点' : lang === 'zhHant' ? '編輯視角' : 'Editorial framing'}</div><h3>${lang === 'ko' ? '좋은 여행은 장소보다 템포를 먼저 맞출 때 생깁니다' : lang === 'ja' ? '良い旅は、チェックリストより先にテンポが決まると整います' : lang === 'zhHant' ? '好的旅程，通常是在清單之前先把節奏對上' : 'Trips usually improve when the tempo lands before the checklist does'}</h3><p>${data.lead}</p></article></section>
...
function getSeasonalEditorialCollections(){
  ...
  function renderHomeSeasonalDesk(){
    ...
    const copy = (lang === 'ko') ? {
      ...
      eyebrow:'季節のデスク', title:'季節と状況から始めるエディトリアル設計', desc:'都市からでなくても始められます。春、雨、真夏の海辺、遅めの街リズムから整える編集です。', guide: lang==='ja' ? '都市ガイド' : lang==='zhHant' ? '城市指南' : 'City guide', sample: lang==='ja' ? 'サンプルルート' : lang==='zhHant' ? '範例路線' : 'Sample route', plan: lang==='ja' ? 'このベースを使う' : lang==='zhHant' ? '使用這個基底' : 'Use this base'
    };
  ...
  function renderMagazineSeasonalDesk(){
    ...
    const copy = (lang === 'ko') ? {
      ...
      eyebrow:'季節の編集システム', title:'この季節に合うベースから先に読む', desc:'注目棚の次に来るのは季節と状況です。旅の温度や時間帯から入ると、都市がもっと早く絞れます。', guide: lang==='ja' ? '都市ガイド' : lang==='zhHant' ? '城市指南' : 'City guide', sample: lang==='ja' ? 'サンプルルート' : lang==='zhHant' ? '範例路線' : 'Sample route', plan: lang==='ja' ? 'Planner へ進む' : lang==='zhHant' ? '接到 Planner' : 'Continue in Planner'
    };
  ...
  function renderTripsSeasonalDesk(){
    ...
    const copy = (lang === 'ko') ? {
      eyebrow:'Seasonal archive', title:'Keep seasonal bases ready to reopen later', desc:'Saving seasonal and situation-led bases inside My Trips makes the next trip easier to start.', guide: lang==='ja' ? '都市ガイド' : lang==='zhHant' ? '城市指南' : 'City guide', sample: lang==='ja' ? 'サンプルルート' : lang==='zhHant' ? '範例路線' : 'Sample route', plan: lang==='ja' ? '自分のベースとして保存' : lang==='zhHant' ? '存成我的基底' : 'Save as my base'
    } : {
      eyebrow:'季節のアーカイブ', title:'あとでまた開ける季節の棚', desc:'My Trips の中に季節や状況ごとのベースを残しておくと、次の旅をもっと早く始められます。', guide: lang==='ja' ? '都市ガイド' : lang==='zhHant' ? '城市指南' : 'City guide', sample: lang==='ja' ? 'サンプルルート' : lang==='zhHant' ? '範例路線' : 'Sample route', plan: lang==='ja' ? '自分のベースとして保存' : lang==='zhHant' ? '存成我的基底' : 'Save as my base'
    };
    ...
    const maps = {
      planner: {
        ...
        ja: {
          ...
          '.dispatch-card .collection-kicker':'快速編輯線',
          ...
          '.top-story-card.soft .eyebrow':'クイックエントリー',
  ...
  editorialData.magazine.ja = {
    ...
    sideKicker:'Magazine の役割', sideTitle:'読むだけで終わらず、そのまま計画へ進むハブ', sideLines:[['City mood first','ランドマークより先に、街区の感触、ペース、合う旅タイプを見せます。'],['Less random browsing','fast city / slow day / food-led のように、今の気分からすぐ入れます。'],['Straight into Planner','気に入った都市やサンプルはそのまま Planner の値へつながります。']], sideButtons:['都市ガイド','保存した旅'],
