  ...
  const editorialData = {
    magazine: {
      ko: {
        ...
        readSample:'샘플 읽기', use플래너:'이 흐름으로 시작',
      ...
      en: {
        ...
        readSample:'Read 샘플 루트', use플래너:'Start with this route',
...
editorialData.example['macau-2n3d-night-lanes'] = { titleKo:'Macau 2박 3일 night lanes', titleEn:'Macau 2N3D Night Lanes', city:'Macau', image:'assets/images/examples/macau-heritage-night.jpg', guide:'macau.html', koLead:'마카오의 old lanes와 짧은 night glow를 조금 더 골목 중심으로 읽는 샘플입니다.', enLead:'A Macau sample that leans more into old lanes and one cleaner night glow instead of broad spectacle.', ko:{ routeShape:'old-lane entry → Taipa bridge → 짧은 night glow', energyControl:'하루를 길게 끌지 않고, 걷는 축 하나와 밤 장면 하나만 남깁니다.', swapNote:'비가 오면 heritage walk를 줄이고 indoor tea / dessert pocket을 더 길게 쓰세요.', bestWith:'커플, 부모님, 짧은 heritage trip', whyBullets:['마카오는 골목을 짧게 읽을수록 더 또렷합니다.','Taipa가 old core와 night side 사이를 부드럽게 이어줍니다.','한 장면의 밤으로도 충분히 contrast가 남습니다.'] }, en:{ routeShape:'Old-lane entry → Taipa bridge → short night glow', energyControl:'Keep the day short and leave only one walking axis plus one night scene in focus.', swapNote:'When rain arrives, reduce the heritage walk and extend one indoor tea or dessert pocket instead.', bestWith:'Couples, parents, short heritage trips', whyBullets:['Macau often gets clearer when its lanes are read briefly and well.','Taipa softens the jump between the old core and the brighter night side.','One night scene is enough to hold the contrast.'] } };
  ...
  function renderMagazineLanding(){
    ...
    const cityMarkup = order.map((key, idx) => {
      ...
      return `<article class="city-card info-card finder-card${leadClass}${releaseClass}" data-country="${loop.country.toLowerCase()}" data-vibe="${(loop.vibe||'').replace(/ /g,' ')}" data-track="${trackList.join(' ')}" data-card-href="../${loop.guide}" data-search="${loop.name.toLowerCase()} ${loop.country.toLowerCase()} ${(loop.vibe||'').toLowerCase()} ${trackList.join(' ')} ${(data.chipMap[key] || []).join(' ').toLowerCase()}" tabindex="0" role="link" aria-label="Open ${loop.name} city guide">
        ...
        <div class="card-body"><div class="meta"><span>${meta}</span>${priorityLabel ? `<span class="finder-priority-pill">${priorityLabel}</span>` : ''}</div><h3 class="card-title">${loop.name}</h3><p class="card-copy">${copy}</p><p class="city-share-hook">${getCityGrowthHook(key, { planner: loop.name }).shareLine}</p><div class="finder-visual-meta-inline"><span class="finder-visual-pill">${primaryTrack || meta}</span>${secondaryTrack ? `<span class="finder-visual-pill is-soft">${secondaryTrack}</span>` : `<span class="finder-visual-pill is-soft">${layerLabel}</span>`}</div><div class="trip-chip-row">${chips}</div><div class="card-rail-note">${railCopy}</div><div class="card-actions city-flow-actions"><a class="soft-btn" href="../${loop.guide}">${data.guideBtn}</a><a class="ghost-btn" href="../${loop.example}">${lang === 'ko' ? '샘플' : lang === 'ja' ? 'sample' : lang === 'zhHant' ? 'sample' : 'Sample'}</a><a class="ghost-btn" href="${plannerUrlForCity(loop.name, { entryKind:'city', entryTitle:loop.name, entryCity:loop.name, entrySource:'magazine-finder' })}">${data.planBtn}</a></div></div>
      </article>`;
    }).join('');
    root.innerHTML = `
  ...
  function renderExpansionCityGuideFront(slug, entry){
    ...
    const readSample = lang === 'ko' ? '샘플 루트 읽기' : lang === 'ja' ? 'サンプルルートを読む' : lang === 'zhHant' ? '閱讀範例路線' : 'Read 샘플 루트';
  ...
  function getExpansionCityDeepDiveCopy(slug){
    const labelMap = {
      ko:{eyebrow:'Expansion city deepening',title:'확장 도시도 여행 리듬으로 읽기',desc:'Osaka, Sapporo, Sendai, Okinawa, Jeju, Gyeongju, Macau는 짧은 방문에서도 첫 방문, 재방문, 날씨, 느린 날, 밤 리듬이 다르게 작동합니다.',first:'First trip',ret:'Return trip',rainy:'Rainy day',slow:'Slower day',night:'Night rhythm',ctaGuide:'샘플 루트 보기',ctaPlan:'이 도시로 시작'},
      en:{eyebrow:'Expansion city deepening',title:'Read the expansion cities by travel rhythm',desc:'The expansion cities now carry a complete first-trip, return-trip, rainy-day, slower-day, and night-rhythm layer.',first:'First trip',ret:'Return trip',rainy:'Rainy day',slow:'Slower day',night:'Night rhythm',ctaGuide:'Read 샘플 루트',ctaPlan:'Start with this city'},
      ja:{eyebrow:'Expansion city deepening',title:'拡張都市を旅のリズムで読む',desc:'拡張都市にも、初回、再訪、雨の日、ゆっくりした日、夜のリズムを追加しました。',first:'First trip',ret:'Return trip',rainy:'Rainy day',slow:'Slower day',night:'Night rhythm',ctaGuide:'サンプルを見る',ctaPlan:'この都市で始める'},
      zhHant:{eyebrow:'Expansion city deepening',title:'用旅行節奏讀擴展城市',desc:'擴展城市加入初訪、再訪、雨天、慢旅行與夜晚節奏的完整閱讀層。',first:'First trip',ret:'Return trip',rainy:'Rainy day',slow:'Slower day',night:'Night rhythm',ctaGuide:'看 sample 路線',ctaPlan:'從這座城市開始'}
    };
    const data = {
  ...
  function renderCityRouteHandoff(slug, entry){
    ...
    const labels = {
      ...
      cityLabel: '01 · Read', sampleLabel: '02 · Sample', plannerLabel: '03 · Build',
      ...
      plannerBtn: lang === 'ko' ? '이 도시부터 시작' : lang === 'ja' ? 'この都市から始める' : lang === 'zhHant' ? '從這座城市開始' : 'Start with this city'
    };
  ...
  function getReleaseCityDeepDiveCopy(slug){
    const labelMap = {
      ko:{eyebrow:'Release city deepening',title:'첫 방문 / 재방문 / 날씨별로 다시 읽기',desc:'핵심 7도시는 같은 일정도 방문 경험, 날씨, 속도, 밤의 무게에 따라 다르게 읽힙니다.',first:'First trip',ret:'Return trip',rainy:'Rainy day',slow:'Slower day',night:'Night rhythm',ctaGuide:'샘플 루트 보기',ctaPlan:'이 도시로 시작'},
      en:{eyebrow:'Release city deepening',title:'Read it by visit type, weather, and night rhythm',desc:'The seven release cities now hold a deeper layer for first visits, returns, rain, slower days, and night weight.',first:'First trip',ret:'Return trip',rainy:'Rainy day',slow:'Slower day',night:'Night rhythm',ctaGuide:'Read 샘플 루트',ctaPlan:'Start with this city'},
      ja:{eyebrow:'Release city deepening',title:'初回・再訪・天気・夜のリズムで読む',desc:'Release 7都市を、初回、再訪、雨、ゆっくりした日、夜の重みで読み分けます。',first:'First trip',ret:'Return trip',rainy:'Rainy day',slow:'Slower day',night:'Night rhythm',ctaGuide:'サンプルを見る',ctaPlan:'この都市で始める'},
      zhHant:{eyebrow:'Release city deepening',title:'用初訪、再訪、天氣與夜晚節奏讀城市',desc:'七個 release 城市加入初訪、再訪、雨天、慢旅行與夜晚比重的閱讀層。',first:'First trip',ret:'Return trip',rainy:'Rainy day',slow:'Slower day',night:'Night rhythm',ctaGuide:'看 sample 路線',ctaPlan:'從這座城市開始'}
    };
    const data = {
...
function getSeasonalEditorialCollections(){
  ...
  function getAtlasText(page='home'){
    const copyMap = {
      home: {
        ko:{eyebrow:'City atlas', title:'주요 도시를 cover부터 샘플 루트까지 한 번에 읽기', desc:'각 도시의 첫 인상, 읽는 포인트, 바로 참고할 sample rhythm을 같은 화면에 붙였습니다.', region:{japan:'Japan edit', korea:'Korea edit', greater:'Greater China edit'}, districts:'District highlights', picks:'Neighborhood picks', sample:'Sample', guide:'Read guide', plan:'Start with this city'},
...
function renderTripsSeasonalDesk(){
    ...
    const copy = (lang === 'ko') ? {
      ...
      eyebrow:'季節のアーカイブ', title:'あとでまた開ける季節の棚', desc:'보관함 の中に季節や状況ごとのベースを残しておくと、次の旅をもっと早く始められます。', guide: lang==='ja' ? '都市ガイド' : lang==='zhHant' ? '城市指南' : 'City guide', sample: lang==='ja' ? 'サンプルルート' : lang==='zhHant' ? '範例路線' : 'Sample', plan: lang==='ja' ? '自分のベースとして保存' : lang==='zhHant' ? '存成我的基底' : 'Save as my base'
    };
