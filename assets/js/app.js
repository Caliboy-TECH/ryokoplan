window.RyokoApp = (() => {
  ...
  function renderMagazineLanding(){
    ...
    root.innerHTML = `
      <section class="magazine-hero-v2 hero-card magazine-led-hero">
  ...
  function renderCityPage(){
    ...
    root.innerHTML = `
      <section class="city-detail-hero hero-card city-hero-polish">
        <div class="city-detail-copy"><span class="eyebrow">${data.eyebrow}</span><h1>${entry.planner}</h1><p class="city-detail-lead">${data.lead}</p><div class="mini-vibe-row">${data.chips.map(ch => `<span class="mini-vibe-chip">${ch}</span>`).join('')}</div><div class="city-editor-note"><strong>${lang === 'ko' ? 'Editor's take' : 'Editor's take'}</strong><span>${data.whyDesc}</span></div><div class="hero-actions hero-actions-strong"><a class="primary-btn" href="${plannerUrlForCity(entry.planner)}">${lang === 'ko' ? entry.planner + ' 여행 짜기' : 'Plan ' + entry.planner}</a><a class="ghost-btn" href="../example/${entry.example}">${lang === 'ko' ? '샘플 일정 보기' : 'See sample plan'}</a></div></div>
        <div class="city-detail-visual"><img src="../${entry.image}" alt="${entry.planner}"><div class="glass-note strong"><strong>${data.why}</strong><span>${data.bestFor}</span></div></div>
      </section>
      ...
      <section class="section city-meta-strip" id="city-overview"><article class="meta-card feature"><span class="meta-label">${lang === 'ko' ? 'Best for' : 'Best for'}</span><span class="meta-value">${data.bestFor}</span></article><article class="meta-card"><span class="meta-label">${lang === 'ko' ? 'Suggested pace' : 'Suggested pace'}</span><span class="meta-value">${data.pace}</span></article><article class="meta-card"><span class="meta-label">${lang === 'ko' ? 'Best season' : 'Best season'}</span><span class="meta-value">${data.season}</span></article></section>
      <section class="section city-reading-grid" id="city-districts"><article class="info-card editorial-panel"><div class="section-head compact"><div><div class="editorial-kicker">${lang === 'ko' ? 'Where to start' : 'Where to start'}</div><h2 class="section-title">${data.focusTitle}</h2><p class="section-desc">${data.focusDesc}</p></div></div><div class="district-grid">${data.districts.map(d => `<article class="district-card info-card"><span class="district-label">${lang === 'ko' ? 'District pick' : 'District pick'}</span><h3>${d[0]}</h3><p>${d[1]}</p></article>`).join('')}</div></article><article class="info-card editorial-panel"><div class="section-head compact"><div><div class="editorial-kicker">${lang === 'ko' ? 'How it feels' : 'How it feels'}</div><h2 class="section-title">${data.foodTitle}</h2><p class="section-desc">${data.foodDesc}</p></div></div><ul class="editorial-bullets">${data.foodBullets.map(item => `<li>${item}</li>`).join('')}</ul><div class="editorial-budget-note"><strong>${lang === 'ko' ? 'Budget feel' : 'Budget feel'}</strong><p>${data.budgetFeel}</p></div></article></section>
      <section class="section" id="city-sample"><div class="section-head"><div><div class="editorial-kicker">${lang === 'ko' ? 'Sample route' : 'Sample route'}</div><h2 class="section-title">${data.sampleTitle}</h2><p class="section-desc">${data.sampleDesc}</p></div></div><article class="example-card info-card example-card-strong"><div class="editorial-kicker">${lang === 'ko' ? 'Sample itinerary' : 'Sample itinerary'}</div><h3 class="editorial-example-title">${entry.planner}</h3><div class="example-summary editorial-summary">${data.sampleDays.map(day => `<div class="summary-line editorial-line"><span>${day[0]}</span><span>${day[1]}</span></div>`).join('')}</div><div class="cta-row cta-row-priority"><a class="primary-btn" href="../example/${entry.example}">${lang === 'ko' ? '전체 샘플 열기' : 'Open full example'}</a><a class="soft-btn" href="${plannerUrlForCity(entry.planner)}">${lang === 'ko' ? '플래너에서 커스텀' : 'Customize in Planner'}</a></div></article></section>
      <section class="section city-reading-grid" id="city-tips"><article class="info-card editorial-panel"><div class="section-head compact"><div><div class="editorial-kicker">${lang === 'ko' ? 'Local notes' : 'Local notes'}</div><h2 class="section-title">${lang === 'ko' ? 'Local tips' : 'Local tips'}</h2><p class="section-desc">${lang === 'ko' ? '실제로 여행 체감을 바꾸는 작은 포인트들입니다.' : 'Small adjustments that noticeably improve the trip.'}</p></div></div><ul class="editorial-bullets">${data.tips.map(item => `<li>${item}</li>`).join('')}</ul></article><article class="info-card editorial-panel"><div class="section-head compact"><div><div class="editorial-kicker">${lang === 'ko' ? 'Before you go' : 'Before you go'}</div><h2 class="section-title">${lang === 'ko' ? 'Keep in mind' : 'Keep in mind'}</h2><p class="section-desc">${lang === 'ko' ? '출발 전에 챙기면 바로 체감되는 것들입니다.' : 'Small prep choices that pay off immediately.'}</p></div></div><ul class="editorial-bullets">${data.keep.map(item => `<li>${item}</li>`).join('')}</ul></article></section>
  ...
  function renderExamplePage(){
    ...
    root.innerHTML = `
      <section class="city-detail-hero hero-card city-hero-polish"><div class="city-detail-copy"><span class="eyebrow">${lang === 'ko' ? 'Plan Example' : 'Plan Example'}</span><h1>${title}</h1><p class="city-detail-lead">${lead}</p><div class="mini-vibe-row"><span class="mini-vibe-chip">${entry.city}</span><span class="mini-vibe-chip">${cityData.pace}</span><span class="mini-vibe-chip">${cityData.bestFor.split(',')[0]}</span></div><div class="city-editor-note"><strong>${lang === 'ko' ? 'Use this as a base' : 'Use this as a base'}</strong><span>${lang === 'ko' ? '리스트를 그대로 복사하기보다, 하루 밀도와 동네 조합을 가져가는 편이 좋습니다.' : 'Use the structure and pacing more than the exact list.'}</span></div><div class="hero-actions hero-actions-strong"><a class="primary-btn" href="${plannerUrlForCity(entry.city)}">${lang === 'ko' ? '플래너에서 커스텀' : 'Customize in Planner'}</a><a class="ghost-btn" href="../city/${entry.guide}">${lang === 'ko' ? '도시 가이드 읽기' : 'Read city guide'}</a></div></div><div class="city-detail-visual"><img src="../${entry.image}" alt="${title}"><div class="route-card dark strong"><strong>${lang === 'ko' ? 'Example itinerary' : 'Example itinerary'}</strong><span>${lang === 'ko' ? '하루별 구조, 예산 감각, 현지 팁까지 한 번에.' : 'Day-by-day structure, budget feel, and local tips in one view.'}</span></div></div></section>
      ...
      <section class="section city-meta-strip"><article class="meta-card feature"><span class="meta-label">${lang === 'ko' ? 'Best for' : 'Best for'}</span><span class="meta-value">${cityData.bestFor}</span></article><article class="meta-card"><span class="meta-label">${lang === 'ko' ? 'Suggested pace' : 'Suggested pace'}</span><span class="meta-value">${cityData.pace}</span></article><article class="meta-card"><span class="meta-label">${lang === 'ko' ? 'Budget feel' : 'Budget feel'}</span><span class="meta-value">${cityData.budgetFeel}</span></article></section>
      <section class="section" id="example-flow"><article class="example-card info-card example-card-strong"><div class="editorial-kicker">${lang === 'ko' ? 'Day by day' : 'Day by day'}</div><h2 class="section-title">${lang === 'ko' ? '이렇게 흐릅니다' : 'How the trip flows'}</h2><div class="example-summary editorial-summary">${sample.map(day => `<div class="summary-line editorial-line"><span>${day[0]}</span><span>${day[1]}</span></div>`).join('')}</div></article></section>
      <section class="section city-reading-grid" id="example-why"><article class="info-card editorial-panel"><div class="section-head compact"><div><div class="editorial-kicker">${lang === 'ko' ? 'Why it lands' : 'Why it lands'}</div><h2 class="section-title">${lang === 'ko' ? 'Why this structure works' : 'Why this structure works'}</h2><p class="section-desc">${lang === 'ko' ? '포인트는 많이 보는 게 아니라 도시를 일관되게 느끼게 만드는 것입니다.' : 'The point is not to do everything, but to make the city feel coherent.'}</p></div></div><ul class="editorial-bullets">${cityData.tips.map(item => `<li>${item}</li>`).join('')}</ul></article><article class="info-card editorial-panel"><div class="section-head compact"><div><div class="editorial-kicker">${lang === 'ko' ? 'Adjust before editing' : 'Adjust before editing'}</div><h2 class="section-title">${lang === 'ko' ? 'Before you customize it' : 'Before you customize it'}</h2><p class="section-desc">${lang === 'ko' ? '커스텀 전에 기준으로 삼기 좋은 포인트입니다.' : 'Good baseline decisions before you customize it.'}</p></div></div><ul class="editorial-bullets">${cityData.keep.map(item => `<li>${item}</li>`).join('')}</ul></article></section>
