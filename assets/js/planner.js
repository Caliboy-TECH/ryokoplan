window.RyokoPlanner = (() => {
  ...
  function priorityCityLoopNote(baseCity='', signals={}, context='result'){
    ...
    const notes = {
      ...
      seoul: {
        result: uiCopy('서울은 같은 날의 동네 대비가 결과를 바꿉니다. 강한 구역 하나와 느슨한 포켓 하나를 같이 보는 쪽이 좋습니다.', 'Seoul reads best when one sharp neighborhood—Seongsu, Euljiro, Hannam—meets one looser pocket that lets the day breathe before night takes over.', 'ソウルは同じ日の街区コントラストで結果が変わります。強い地区とゆるいポケットをひとつずつ置くのがきれいです。', '首爾的結果很常被同一天裡的街區對比拉開。放一個強區域，再配一個鬆一點的 pocket 會更好。'),
      ...
      busan: {
        result: uiCopy('부산은 바다를 보는 시간대와 쉬는 창이 같이 잡혀야 좋습니다. 전망 하나와 느린 식사 하나를 같이 보세요.', 'Busan works best when the coast arrives at the right hour and one true rest window keeps the route from turning into slopes and transfers.', '釜山は海を見る時間帯と休む窓が一緒に取れているときがいちばんきれいです。', '釜山最適合把看海的時間帶和真正的休息窗口一起留住。'),
  ...
  const priorityCityRefineMap = {
    ...
    hongkong:{
      ...
      en:{district:'Hong Kong becomes cleaner once you choose whether Central-Sheung Wan, Tsim Sha Tsui, or Wan Chai should own the vertical rhythm for the day.', rhythm:[['Vertical start','Open through a steeper, more compressed axis like Central or Sheung Wan.'],['Breathing pocket','One ferry or covered-mall pause keeps the route alive into the evening.'],['Harbor close','One harbor scene is enough when it is placed well.']], variants:[['Rainy','Lean on covered malls, ferry logic, and indoor meals more confidently.'],['Slower','Do not read both Central and Tsim Sha Tsui deeply on the same day.'],['Night','After one skyline moment, lower the tone through a slope street or dessert pocket.']]}
    },
    busan:{
