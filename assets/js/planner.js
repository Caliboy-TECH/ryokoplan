window.RyokoPlanner = (() => {
  ...
  const secondaryCityThinEntryMap = {
    ...
    ,sendai:{ko:{visitTitle:'Visit lens', first:['First trip','Station → arcade','중심 축부터 가볍게.'], second:['Return trip','Jozenji-dori → river edge','조용한 북측 pace가 더 잘 남습니다.'], entries:[['Centre','Station → arcade'],['Calm','Jozenji-dori → river edge']]}, en:{visitTitle:'Visit lens', first:['First trip','Station → arcade','Open through the clearest central line first.'], second:['Return trip','Jozenji-dori → river edge','A slower north-side rhythm usually lands better on a return.'], entries:[['Centre','Station → arcade'],['Calm','Jozenji-dori → river edge']]}}
...
function getPriorityRefinePack(city=''){
  ...
  function resultCopy(){
    return {
      ...
      sampleRead: uiCopy('샘플 보기', 'Sample route', 'サンプルを見る', '查看範例'),
      ...
      readCityLayer: uiCopy('도시 결 더 읽기', 'City guide layer', '都市の層を読む', '繼續讀城市層次'),
